...
const entries = Object.entries(PEOPLE).sort(([,a],[,b]) => (a.name || "").localeCompare(b.name || "", "nl"));
...
function render(query = "") {
  ...
  grid.replaceChildren(...filtered.map(([id,person]) => {
    ...
    body.append(title,dates); link.append(image,body); card.append(link); return card;
  }));
